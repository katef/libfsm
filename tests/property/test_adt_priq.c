...
static bool op_pop(struct model *m, struct priq_op *op, struct priq **p);
static bool op_push(struct model *m, struct priq_op *op, struct priq **p);
static bool op_update(struct model *m, struct priq_op *op, struct priq **p);
static bool op_move(struct model *m, struct priq_op *op, struct priq **p);
static bool model_verify(struct model *m);
...
repeat_with_verbose(const struct theft_hook_trial_post_info *info,
    void *venv)
{
	struct priq_hook_env *env = (struct priq_hook_env *)venv;
	assert(env->tag == 'p');
...
static bool test_priq_operations(uintptr_t limit)
{
	(void)limit;
	struct priq_hook_env env = {
		.tag = 'p',
 ...
 *********************************************/
	...
	struct theft_run_config config = {
		...
		.seed = seed,
		.always_seed_count = sizeof(always_seeds)/sizeof(always_seeds[0]),
...
prop_priq_operations(struct theft *t, void *arg1)
{
	struct priq_scenario *s = (struct priq_scenario *)arg1;
	struct priq_hook_env *env = theft_hook_get_env(t);
	assert(env->tag == 'p');
	(void)s;
	(void)env;
	...
	m->records = calloc(s->count, sizeof(struct record));
	if (m->records == NULL) { return THEFT_TRIAL_ERROR; }
	m->env = env;
	m->count = s->count;
	...
	/* run operations */
	for (size_t op_i = 0; op_i < s->count; op_i++) {
		struct priq_op *op = (struct priq_op *)&s->ops[op_i];
		switch (op->t) {
		case PRIQ_OP_POP:
			if (!op_pop(m, op, &p)) { return THEFT_TRIAL_FAIL; }
			break;

		case PRIQ_OP_PUSH:
			if (!op_push(m, op, &p)) { return THEFT_TRIAL_FAIL; }
			break;
		
		case PRIQ_OP_UPDATE:
			if (!op_update(m, op, &p)) { return THEFT_TRIAL_FAIL; }
			break;

		case PRIQ_OP_MOVE:
			if (!op_move(m, op, &p)) { return THEFT_TRIAL_FAIL; }
			break;
	...
	if (!model_verify(m)) { return THEFT_TRIAL_FAIL; }
...
static bool op_pop(struct model *m, struct priq_op *op, struct priq **p)
{
	...
	if (m->used == 0) {
		...
		for (size_t i = 0; i < m->used; i++) {
			struct record *r = &m->records[i];
			if (lowest > r->cost) { lowest = r->cost; }
			if (m->env->verbosity > 0) {
			...
			if (r->cost == popped->cost && r->id == (uintptr_t)popped->state && r->entry == popped) {
				found = true;
				ri = i;
				if (m->env->verbosity > 0) { fprintf(stderr, "HIT\n"); }
				break;
			}
		}
		...
		if (ri < m->used - 1) {
			...
			memcpy(&m->records[ri], &m->records[m->used - 1],
...
static bool op_push(struct model *m, struct priq_op *op, struct priq **p)
{
	...
	struct fsm_state *fake_state = fake_state_of_id(op->u.push.id);
	struct priq *old_p = *p;
	struct priq *new_node = priq_push(p, fake_state, op->u.push.cost);
	if (m->env->verbosity > 0) {
	...
	for (size_t i = 0; i < m->used; i++) {
		struct record *r = &m->records[i];
		if (r->cost < lowest) { lowest = r->cost; }
	}
	...
	struct priq *entry = priq_find(*p, fake_state);
...
void register_test_adt_priq(void)
{
	...
	//reg_test1("adt_priq_operations", test_priq_operations, 10000);
}
