...
static const char literals[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789____";
    ...
    struct test_re_info *info)
{
	...
	if (info->u.pcre.head == NULL) { return false; }
	...
	if (info->pos_pairs == NULL) { return false; }
	for (size_t i = 0; i < info->pos_count; i++) {
		uint8_t *positive = gen_expected_match(t, info->u.pcre.head,
	...
	if (info->neg_pairs == NULL) { return false; }
	for (size_t i = 0; i < info->neg_count; i++) {
		struct error_counter bomb = {
			.counter = theft_random_bits(t, 4),
		};
		uint8_t *negative = gen_expected_match(t, info->u.pcre.head,
...
gen_pcre_node(struct theft *t)
{
	...
	uint64_t buf64[256 / sizeof(uint64_t)] = { [0] = 0 };
	uint8_t *buf = (uint8_t *)buf64;

	struct pcre_node *n = calloc(1, sizeof(*n));
...
/* Append another string buffer, with parens around it if it's more than
 * a single character. */
static bool pop_and_append(struct flatten_env *env)
{
	struct buf *popped = buf_pop(env->b);
	assert(popped);

	if (popped->size > 0) {
		bool parens = popped->size > 1;
		if (parens) {
			if (!buf_append(env->b, '(', false)) { return false; }
		}
		
		if (!buf_grow(env->b, env->b->size + popped->size)) { return false; }
...
static bool flatten(struct flatten_env *env, const struct pcre_node *node)
{
	#ifdef LOG_VERBOSE
	...
	case PN_DOT:
		...
		break;
	case PN_LITERAL:
		if (!buf_grow(env->b, env->b->size + node->u.literal.size)) { return false; }
		buf_memcpy(env->b, (uint8_t *)node->u.literal.string,
		    node->u.literal.size);
		break;
	case PN_QUESTION:
		if (!buf_push(env->b)) { return false; }

		if (!flatten(env, node->u.question.inner)) { return false; }
		if (!pop_and_append(env)) { return false; }
	...
	case PN_KLEENE:
		...
		if (!flatten(env, node->u.kleene.inner)) { return false; }
		if (!pop_and_append(env)) { return false; }
	...
	case PN_PLUS:
		...
		if (!flatten(env, node->u.plus.inner)) { return false; }
		if (!pop_and_append(env)) { return false; }
	...
	case PN_BRACKET:
		...
		if (!bracket_consumes_any(node)) { break; } /* skip */
		...
		for (size_t i = 0; i < 256; i++) {
			if (node->u.bracket.set[i / 64] & (1LLU << (i % 64))) {
				...
				if (!buf_append(env->b, c, true)) { return false; }
			}
		}
...
static bool bracket_consumes_any(const struct pcre_node *node)
{
	...
	for (uint8_t i = 0; i < 4; i++) {
		uint64_t v = node->u.bracket.set[i];
		if (node->u.bracket.negated) { v = ~ v; }
		if (v > 0) { return true; }
	}
	return false;
}
    ...
    size_t *psize)
{
	...
	if (env.b == NULL) { return NULL; }

	if (flatten(&env, re_tree)) {
		if (!buf_grow(env.b, env.b->size + 1)) { return false; }
		*psize = env.b->size;
		uint8_t *res = malloc(env.b->size + 1);
		assert(res);
...
static bool in_set(const uint64_t *set, uint8_t pos) {
	return 0 != (set[pos / 64] & (1LLU << (pos % 64)));
}

static void counter_tick(struct error_counter *bomb)
{
...
static bool counter_timed_out(struct error_counter *bomb)
{
	if (bomb == NULL) { return false; }
	return !bomb->used && bomb->counter == 0;
}

static void counter_used(struct error_counter *bomb)
{
	bomb->used = true;
}

static bool build_exp_match(struct theft *t,
    ...
    struct error_counter *bomb)
{
	...
	case PN_BRACKET:
	{
		...
		if (node->u.bracket.class_flags != 0x0000 && theft_random_bits(t, 1)) {
			if (!build_exp_class_flag(t, buf, node, bomb)) { return false; }
			break;
		}

		uint8_t start = theft_random_bits(t, 8);
		/* Pick the first matching character in the set */
		const uint64_t *set = node->u.bracket.set;
		bool any = false;
		bool neg = node->u.bracket.negated;
		if (counter_timed_out(bomb)) {
			counter_used(bomb);
			neg = !neg;
		} else {
	...
	case PN_ALT:
	{
		...
		uint8_t choice = theft_random_choice(t, node->u.alt.count);
		if (!build_exp_match(t, buf,
    ...
    const struct pcre_node *node, struct error_counter *bomb)
{
	...
	bool inject_error = counter_timed_out(bomb);

	uint8_t byte = 0x00;

	const uint8_t class_count = BRACKET_CLASS_COUNT;
	assert(((1LLU << class_count) - 1) == BRACKET_CLASS_MASK);
	uint8_t offset = theft_random_bits(t, 4);
	for (uint8_t i = 0; i < class_count; i++) {
		enum pcre_bracket_class class =
		    1LLU << ((offset + i) % class_count);
		if (node->u.bracket.class_flags & class) {
			switch (class) {
			default:
				assert(false);
			...
			if (!buf_append(buf, byte, false)) { return false; }
			return true;
		}
	}
	assert(false);
...
static uint8_t *gen_expected_match(struct theft *t,
...
static void free_pcre_tree(struct pcre_node *node) {
	switch (node->t) {
	case PN_LITERAL:
