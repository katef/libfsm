...
run_test(const struct eager_output_test *test)
{
	...
	if (test->force_endids) {
		...
		/* This function sets the end & eager output IDs. */
	...
	for (size_t i_i = 0; i_i < MAX_INPUTS; i_i++) {
		...
		ret = fsm_exec(fsm, fsm_sgetc, &input, &end);
...
#define ENDID_BUF_SIZE 32
			...
			const enum fsm_getendids_res egres = fsm_endid_get(fsm, end, /*ENDID_BUF_SIZE*/ endid_count, endid_buf);
			switch (egres) {
			case FSM_GETENDIDS_NOT_FOUND:
				/* "not found" can be fine if it only has eager outputs */
				break;
			case FSM_GETENDIDS_FOUND:
				break;
			case FSM_GETENDIDS_ERROR_INSUFFICIENT_SPACE:
			default:
				fprintf(stderr, "fsm_endid_get res %d\n", egres);
