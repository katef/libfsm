...
static struct interned_state_set *
add_states_from_array(struct interned_state_set_pool *issp,
    size_t length, const fsm_state_t *states, unsigned verbosity)
{
	size_t i;
	struct interned_state_set *empty = interned_state_set_empty(issp);
	struct interned_state_set *cur, *prev;
	...
	for (i = 0; i < length; i++) {
		...
		cur = interned_state_set_add(issp, cur, s);
...
int main(void) {
	struct interned_state_set *set_up, *set_up2, *set_down, *set_down2;
	fsm_state_t i;
	...
	states = malloc(limit * sizeof(states[0]));
	assert(states != NULL);
	...
	for (i = 0; i < shuffle_cycles; i++) {
		struct interned_state_set *shuffled;
