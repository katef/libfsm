	...
	size_t *u)
{
	...
	groups = f_malloc(fsm->opt->allocator, sizeof *groups * n); /* worst case */
	if (groups == NULL) {
		return NULL;
	}
	...
	while (i < n) {
		...
		p = f_malloc(fsm->opt->allocator, sizeof *p * (n - i)); /* worst case */
		if (p == NULL) {
			j++;
			goto error;
		}
...

		{
			...
			tmp = f_realloc(fsm->opt->allocator, p, sizeof *p * k);
			if (tmp == NULL) {
				j++;
				goto error;
			}
	...
	*u = j;

	{
		...
		tmp = f_realloc(fsm->opt->allocator, groups, sizeof *groups * j);
		if (tmp == NULL) {
			goto error;
		}
...
error:
	...
	for (i = 0; i < j; i++) {
		f_free(fsm->opt->allocator, (void *) groups[i].ranges);
	}

	f_free(fsm->opt->allocator, groups);
...
free_groups(const struct fsm *fsm, struct ir_group *groups, size_t n)
{
	...
	for (j = 0; j < n; j++) {
		f_free(fsm->opt->allocator, (void *) groups[j].ranges);
	}
...
make_holes(const struct fsm *fsm, const struct bm *bm, size_t *n)
{
	...
	ranges = f_malloc(fsm->opt->allocator, sizeof *ranges * FSM_SIGMA_COUNT); /* worst case */
	if (ranges == NULL) {
		return NULL;
	}
	...
	*n = 0;
...

	{
		...
		tmp = f_realloc(fsm->opt->allocator, ranges, sizeof *ranges * *n);
		if (tmp == NULL) {
			goto error;
		}
...
error:
	...
	f_free(fsm->opt->allocator, ranges);
	...
	const struct ir *ir, struct ir_state *cs)
{
	...
	if (hole < max.n) {
		...
		f_free(fsm->opt->allocator, (void *) groups[max.j].ranges);
		if (max.j < n) {
...
make_ir(const struct fsm *fsm)
{
	...
	ir = f_malloc(fsm->opt->allocator, sizeof *ir);
	if (ir == NULL) {
		return NULL;
	}

	ir->n      = fsm_countstates(fsm);
	ir->states = f_malloc(fsm->opt->allocator, sizeof *ir->states * ir->n);
	if (ir->states == NULL) {
		f_free(fsm->opt->allocator, ir);
		return NULL;
	}
	...
	for (s = fsm->sl, i = 0; s != NULL; s = s->next, i++) {
		...
		if (!fsm->opt->comments) {
			 ...
			 */
			p = f_malloc(fsm->opt->allocator, ir->n + 3 + 1);
			if (p == NULL) {
				goto error_example;
			}
			...
			if (n == 0) {
				...
				tmp = f_realloc(fsm->opt->allocator, p, n + 1);
				if (tmp == NULL) {
					f_free(fsm->opt->allocator, p);
					goto error_example;
				}
...
free_ir(const struct fsm *fsm, struct ir *ir)
{
	...
	for (i = 0; i < ir->n; i++) {
		f_free(fsm->opt->allocator, (void *) ir->states[i].example);
		...
		case IR_ERROR:
			f_free(fsm->opt->allocator, (void *) ir->states[i].u.error.error.ranges);
	...
	f_free(fsm->opt->allocator, ir->states);
