...
struct analysis_info {
	 ...
	 * copied from linkage_info. This is used to identify where the input
	 * FSMs' original unanchored end loops should be disconnected and
	 * replaced with a shared end loop. */
...
analyze_group_nfa(const struct fsm *nfa, struct analysis_info *ainfo)
{
	...
	struct state_set **eclosures = fsm_epsilon_closure(nfa);
	if (eclosures == NULL) {
		return false;
	}
...
#if LOG_ANALYZE_GROUP_NFA_RESULTS
	{
		fprintf(stderr, "# analysis_info start %d, usl %d, uel %d, eai %d\n",
		    ainfo->start, ainfo->unanchored_start_loop, ainfo->unanchored_end_loop, ainfo->end_any_inner);
...
{
	const bool nullable_and_end_any_inner = ainfo->nullable
	    && ainfo->end_any_inner != NO_STATE;

	const bool log = LOG_ANALYZE_GROUP_NFA > 0;

	/* Add the eager match state if there are eager match states
	 * or a nullable unanchored end. This will link to the global NFA's
	 * unanchored_end_loop. */
	if (!state_set_empty(ainfo->eager_matches) || nullable_and_end_any_inner) {
		...
		while (state_set_next(&si, &ems_i)) {
			...
			if (!replace_labeled_edge(nfa, ems_i,
				ainfo->end_any_inner, ainfo->eager_match_state, &found)) {
				return false;
			}
			 ...
			 * combining more than ~10 NFAs incredibly expensive. */
			struct edge_group_iter iter;
			struct edge_group_iter_info info;
	...
	if (!state_set_empty(ainfo->anchored_ends)) {
		...
		while (state_set_next(&si, &anchored_end_state)) {
			if (!fsm_isend(nfa, anchored_end_state)) {
				fprintf(stderr, "%s: anchored_end_state %d: NOT end\n", __func__, anchored_end_state);
				assert(fsm_isend(nfa, anchored_end_state));
			}

			if (fsm_endid_set(nfa, anchored_end_state, end_id) < 0) {
				return false;
			}
			if (log) {
    ...
    struct fsm **nfas, struct fsm_combined_base_pair *bases, size_t id_base)
{
	...
	for (size_t i = 0; i < nfa_count; i++) {
		 ...
		 * freed during the call to fsm_merge below. */
		{
			...
			ainfo->end_any_inner = linkage_info->end_any_inner;
