/* Generated by lx */

#include <assert.h>
#include <stdio.h>
#include <stddef.h>
#include <stdlib.h>
#include <errno.h>

#include LX_HEADER

static enum lx_pcre_token z0(struct lx_pcre_lx *lx);
static enum lx_pcre_token z1(struct lx_pcre_lx *lx);
static enum lx_pcre_token z2(struct lx_pcre_lx *lx);
static enum lx_pcre_token z3(struct lx_pcre_lx *lx);
static enum lx_pcre_token z4(struct lx_pcre_lx *lx);
static enum lx_pcre_token z5(struct lx_pcre_lx *lx);
static enum lx_pcre_token z6(struct lx_pcre_lx *lx);
static enum lx_pcre_token z7(struct lx_pcre_lx *lx);

static int
lx_pcre_advance_end(struct lx_pcre_lx *lx, int c)
{
	lx->end.byte++;
	lx->end.col++;
	if (c == '\n') {
		lx->end.line++;
		lx->end.saved_col = lx->end.col - 1;
		lx->end.col = 1;
	}
	if (lx->push != NULL) {
		if (-1 == lx->push(lx->buf_opaque, (char)c)) {
			return 0;
		}
	}
	return 1;
}

/* This wrapper manages one character of lookahead/pushback
 * and the line, column, and byte offsets. */
#if __STDC_VERSION__ >= 199901L
inline
#endif
static int
lx_pcre_getc(struct lx_pcre_lx *lx)
{
	int c;

	assert(lx != NULL);
	assert(lx->lgetc != NULL);

	if (lx->c != EOF) {
		c = lx->c, lx->c = EOF;
	} else {
		c = lx->lgetc(lx);
		if (c == EOF) {
			return EOF;
		}
	}

	if (!lx_pcre_advance_end(lx, c)) { return EOF; }

	return c;
}

/* This wrapper adapts calling lx_pcre_getc to the interface
 * in libfsm's generated code. */
static int
fsm_getc(void *getc_opaque)
{
	return lx_pcre_getc((struct lx_pcre_lx *)getc_opaque);
}

#if __STDC_VERSION__ >= 199901L
inline
#endif
static void
lx_pcre_ungetc(struct lx_pcre_lx *lx, int c)
{
	assert(lx != NULL);
	assert(lx->c == EOF);
	lx->c = c;
	lx->end.byte--;
	lx->end.col--;

	if (c == '\n') {
		lx->end.line--;
		lx->end.col = lx->end.saved_col;
	}
}

int
lx_pcre_dynpush(void *buf_opaque, char c)
{
	struct lx_dynbuf *t = buf_opaque;

	assert(t != NULL);

	if (t->a == NULL || t->p == t->a + t->len) {
		size_t len;
		ptrdiff_t off;
		char *tmp;

		if (t->len == 0) {
			assert(LX_DYN_LOW > 0);
			len = LX_DYN_LOW;
		} else {
			len = t->len * LX_DYN_FACTOR;
			if (len < t->len) {
				errno = ERANGE;
				return -1;
			}
		}

		off = t->p - t->a;
		tmp = realloc(t->a, len);
		if (tmp == NULL) {
			return -1;
		}

		t->p   = tmp + off;
		t->a   = tmp;
		t->len = len;
	}

	assert(t->p != NULL);
	assert(t->a != NULL);

	*t->p++ = c;

	return 0;
}

static void
lx_pcre_dynpop(void *buf_opaque)
{
	struct lx_dynbuf *t = buf_opaque;

	assert(t != NULL);

	assert(t->p != t->a);
	t->p--;
}

int
lx_pcre_dynclear(void *buf_opaque)
{
	struct lx_dynbuf *t = buf_opaque;

	assert(t != NULL);

	if (t->len > LX_DYN_HIGH) {
		size_t len;
		char *tmp;

		len = t->len / LX_DYN_FACTOR;

		tmp = realloc(t->a, len);
		if (tmp == NULL) {
			return -1;
		}

		t->a   = tmp;
		t->len = len;
	}

	t->p = t->a;

	return 0;
}

void
lx_pcre_dynfree(void *buf_opaque)
{
	struct lx_dynbuf *t = buf_opaque;

	assert(t != NULL);

	free(t->a);
}
static enum lx_pcre_token
z0(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '\\': state = S1; break;
			case '\x00': 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			default: state = S2; break;
			}
			break;

		case S1: /* e.g. "\\" */
			switch ((unsigned char) c) {
			case 'E': state = S3; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;
			}
			break;

		case S2: /* e.g. "\\x01" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;

		case S3: /* e.g. "\\E" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, lx->z(lx);

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_CHAR;
	case S2: return TOK_CHAR;
	case S3: return lx->z = z7, lx->z(lx);
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S3:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z1(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': state = S1; break;
			case ',': state = S2; break;
			case '}': state = S3; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S1: /* e.g. "0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_COUNT;
			}
			break;

		case S2: /* e.g. "," */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_SEP;

		case S3: /* e.g. "}" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, TOK_CLOSECOUNT;

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_COUNT;
	case S2: return TOK_SEP;
	case S3: return lx->z = z7, TOK_CLOSECOUNT;
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		if (lx->push != NULL) {
			if (-1 == lx->push(lx->buf_opaque, (char)c)) {
				return TOK_ERROR;
			}
		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z2(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '\\': state = S1; break;
			case '\x00': 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			default: state = S2; break;
			}
			break;

		case S1: /* e.g. "\\" */
			switch ((unsigned char) c) {
			case 'E': state = S3; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;
			}
			break;

		case S2: /* e.g. "\\x01" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;

		case S3: /* e.g. "\\E" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z3, lx->z(lx);

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_CHAR;
	case S2: return TOK_CHAR;
	case S3: return lx->z = z3, lx->z(lx);
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S3:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z3(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3, S4, S5, S6, S7, S8, S9, 
		S10, S11, S12, S13, S14, S15, S16, S17, S18, S19, 
		S20, S21, S22, S23, S24, S25, S26, S27, S28, S29, 
		S30, S31, S32, S33, S34, S35, S36, S37, S38, S39, 
		S40, S41, S42, S43, S44, S45, S46, S47, S48, S49, 
		S50, S51, S52, S53, S54, S55, S56, S57, S58, S59, 
		S60, S61, S62, S63, S64, S65, S66, S67, S68, S69, 
		S70, S71, S72
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '\\': state = S1; break;
			case '[': state = S2; break;
			case '-': state = S4; break;
			case ']': state = S5; break;
			case '\x00': 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			default: state = S3; break;
			}
			break;

		case S1: /* e.g. "\\" */
			switch ((unsigned char) c) {
			case 'D':
			case 'H':
			case 'S':
			case 'V':
			case 'W':
			case 'd':
			case 'h':
			case 's':
			case 'v':
			case 'w': state = S23; break;
			case 'c': state = S55; break;
			case '$':
			case '(':
			case '*':
			case '+':
			case '-':
			case '.':
			case '?':
			case '[':
			case '\\':
			case '^':
			case 'a':
			case 'b':
			case 'e':
			case 'f':
			case 'n':
			case 'r':
			case 't':
			case '{':
			case '|': state = S56; break;
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': state = S57; break;
			case 'Q': state = S58; break;
			case 'E': state = S59; break;
			case 'o': state = S60; break;
			case 'x': state = S62; break;
			case '0': state = S63; break;
			default: state = S61; break;
			}
			break;

		case S2: /* e.g. "[" */
			switch ((unsigned char) c) {
			case ':': state = S7; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;
			}
			break;

		case S3: /* e.g. "\\x01" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;

		case S4: /* e.g. "-" */
			switch ((unsigned char) c) {
			case ']': state = S6; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_RANGE;
			}
			break;

		case S5: /* e.g. "]" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, TOK_CLOSEGROUP;

		case S6: /* e.g. "-]" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, TOK_CLOSEGROUPRANGE;

		case S7: /* e.g. "[:" */
			switch ((unsigned char) c) {
			case 'd': state = S8; break;
			case 'p': state = S9; break;
			case 'x': state = S10; break;
			case 'c': state = S11; break;
			case 'l': state = S12; break;
			case 'g': state = S13; break;
			case 's': state = S14; break;
			case 'a': state = S15; break;
			case 'b': state = S16; break;
			case 'u': state = S17; break;
			case 'w': state = S18; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S8: /* e.g. "[:d" */
			switch ((unsigned char) c) {
			case 'i': state = S53; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S9: /* e.g. "[:p" */
			switch ((unsigned char) c) {
			case 'r': state = S48; break;
			case 'u': state = S49; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S10: /* e.g. "[:x" */
			switch ((unsigned char) c) {
			case 'd': state = S8; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S11: /* e.g. "[:c" */
			switch ((unsigned char) c) {
			case 'n': state = S45; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S12: /* e.g. "[:l" */
			switch ((unsigned char) c) {
			case 'o': state = S44; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S13: /* e.g. "[:g" */
			switch ((unsigned char) c) {
			case 'r': state = S41; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S14: /* e.g. "[:s" */
			switch ((unsigned char) c) {
			case 'p': state = S38; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S15: /* e.g. "[:a" */
			switch ((unsigned char) c) {
			case 's': state = S30; break;
			case 'l': state = S31; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S16: /* e.g. "[:b" */
			switch ((unsigned char) c) {
			case 'l': state = S27; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S17: /* e.g. "[:u" */
			switch ((unsigned char) c) {
			case 'p': state = S24; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S18: /* e.g. "[:w" */
			switch ((unsigned char) c) {
			case 'o': state = S19; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S19: /* e.g. "[:wo" */
			switch ((unsigned char) c) {
			case 'r': state = S20; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S20: /* e.g. "[:wor" */
			switch ((unsigned char) c) {
			case 'd': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S21: /* e.g. "[:word" */
			switch ((unsigned char) c) {
			case ':': state = S22; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S22: /* e.g. "[:word:" */
			switch ((unsigned char) c) {
			case ']': state = S23; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S23: /* e.g. "\\D" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NAMED__CLASS;

		case S24: /* e.g. "[:up" */
			switch ((unsigned char) c) {
			case 'p': state = S25; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S25: /* e.g. "[:low" */
			switch ((unsigned char) c) {
			case 'e': state = S26; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S26: /* e.g. "[:lowe" */
			switch ((unsigned char) c) {
			case 'r': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S27: /* e.g. "[:bl" */
			switch ((unsigned char) c) {
			case 'a': state = S28; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S28: /* e.g. "[:bla" */
			switch ((unsigned char) c) {
			case 'n': state = S29; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S29: /* e.g. "[:blan" */
			switch ((unsigned char) c) {
			case 'k': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S30: /* e.g. "[:as" */
			switch ((unsigned char) c) {
			case 'c': state = S36; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S31: /* e.g. "[:al" */
			switch ((unsigned char) c) {
			case 'p': state = S32; break;
			case 'n': state = S33; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S32: /* e.g. "[:alp" */
			switch ((unsigned char) c) {
			case 'h': state = S35; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S33: /* e.g. "[:aln" */
			switch ((unsigned char) c) {
			case 'u': state = S34; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S34: /* e.g. "[:alnu" */
			switch ((unsigned char) c) {
			case 'm': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S35: /* e.g. "[:alph" */
			switch ((unsigned char) c) {
			case 'a': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S36: /* e.g. "[:asc" */
			switch ((unsigned char) c) {
			case 'i': state = S37; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S37: /* e.g. "[:asci" */
			switch ((unsigned char) c) {
			case 'i': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S38: /* e.g. "[:sp" */
			switch ((unsigned char) c) {
			case 'a': state = S39; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S39: /* e.g. "[:spa" */
			switch ((unsigned char) c) {
			case 'c': state = S40; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S40: /* e.g. "[:spac" */
			switch ((unsigned char) c) {
			case 'e': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S41: /* e.g. "[:gr" */
			switch ((unsigned char) c) {
			case 'a': state = S42; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S42: /* e.g. "[:gra" */
			switch ((unsigned char) c) {
			case 'p': state = S43; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S43: /* e.g. "[:grap" */
			switch ((unsigned char) c) {
			case 'h': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S44: /* e.g. "[:lo" */
			switch ((unsigned char) c) {
			case 'w': state = S25; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S45: /* e.g. "[:cn" */
			switch ((unsigned char) c) {
			case 't': state = S46; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S46: /* e.g. "[:cnt" */
			switch ((unsigned char) c) {
			case 'r': state = S47; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S47: /* e.g. "[:cntr" */
			switch ((unsigned char) c) {
			case 'l': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S48: /* e.g. "[:pr" */
			switch ((unsigned char) c) {
			case 'i': state = S52; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S49: /* e.g. "[:pu" */
			switch ((unsigned char) c) {
			case 'n': state = S50; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S50: /* e.g. "[:pun" */
			switch ((unsigned char) c) {
			case 'c': state = S51; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S51: /* e.g. "[:digi" */
			switch ((unsigned char) c) {
			case 't': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S52: /* e.g. "[:pri" */
			switch ((unsigned char) c) {
			case 'n': state = S51; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S53: /* e.g. "[:di" */
			switch ((unsigned char) c) {
			case 'g': state = S54; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S54: /* e.g. "[:dig" */
			switch ((unsigned char) c) {
			case 'i': state = S51; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S55: /* e.g. "\\c" */
			state = S72; break;

		case S56: /* e.g. "\\$" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_ESC;

		case S57: /* e.g. "\\1" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S58: /* e.g. "\\Q" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z2, lx->z(lx);

		case S59: /* e.g. "\\E" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z(lx);

		case S60: /* e.g. "\\o" */
			switch ((unsigned char) c) {
			case '{': state = S70; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NOESC;
			}
			break;

		case S61: /* e.g. "\\\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NOESC;

		case S62: /* e.g. "\\x" */
			switch ((unsigned char) c) {
			case '{': state = S66; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S67; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;
			}
			break;

		case S63: /* e.g. "\\0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S64; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;
			}
			break;

		case S64: /* e.g. "\\00" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S65; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;
			}
			break;

		case S65: /* e.g. "\\000" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;

		case S66: /* e.g. "\\x{" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S69; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S67: /* e.g. "\\x0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S68; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;
			}
			break;

		case S68: /* e.g. "\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;

		case S69: /* e.g. "\\x{0" */
			switch ((unsigned char) c) {
			case '}': state = S68; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S70: /* e.g. "\\o{" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S71; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S71: /* e.g. "\\o{0" */
			switch ((unsigned char) c) {
			case '}': state = S65; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S72: /* e.g. "\\c\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CONTROL;

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_INVALID;
	case S2: return TOK_CHAR;
	case S3: return TOK_CHAR;
	case S4: return TOK_RANGE;
	case S5: return lx->z = z7, TOK_CLOSEGROUP;
	case S6: return lx->z = z7, TOK_CLOSEGROUPRANGE;
	case S23: return TOK_NAMED__CLASS;
	case S55: return TOK_NOESC;
	case S56: return TOK_ESC;
	case S57: return TOK_UNSUPPORTED;
	case S58: return lx->z = z2, lx->z(lx);
	case S59: return TOK_EOF;
	case S60: return TOK_NOESC;
	case S61: return TOK_NOESC;
	case S62: return TOK_HEX;
	case S63: return TOK_OCT;
	case S64: return TOK_OCT;
	case S65: return TOK_OCT;
	case S67: return TOK_HEX;
	case S68: return TOK_HEX;
	case S72: return TOK_CONTROL;
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S58:
		case S59:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z4(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3, S4, S5, S6, S7, S8, S9, 
		S10
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '+':
			case 'R': state = S1; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': state = S2; break;
			case 'n': state = S3; break;
			case 'x': state = S4; break;
			case 's': state = S5; break;
			case 'i': state = S6; break;
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f':
			case 'g':
			case 'h':
			case 'j':
			case 'k':
			case 'l':
			case 'm':
			case 'o':
			case 'p':
			case 'q':
			case 'r':
			case 't':
			case 'u':
			case 'v':
			case 'w':
			case 'y':
			case 'z': state = S7; break;
			case '-': state = S8; break;
			case ')': state = S9; break;
			case ':': state = S10; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S1: /* e.g. "+" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;

		case S2: /* e.g. "0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S3: /* e.g. "n" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_FLAG__IGNORE;

		case S4: /* e.g. "x" */
			switch ((unsigned char) c) {
			case 'x': state = S7; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_FLAG__EXTENDED;
			}
			break;

		case S5: /* e.g. "s" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_FLAG__SINGLE;

		case S6: /* e.g. "i" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_FLAG__INSENSITIVE;

		case S7: /* e.g. "a" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_FLAG__UNKNOWN;

		case S8: /* e.g. "-" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NEGATE;

		case S9: /* e.g. ")" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, TOK_CLOSE;

		case S10: /* e.g. ":" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, TOK_SUB;

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_UNSUPPORTED;
	case S2: return TOK_UNSUPPORTED;
	case S3: return TOK_FLAG__IGNORE;
	case S4: return TOK_FLAG__EXTENDED;
	case S5: return TOK_FLAG__SINGLE;
	case S6: return TOK_FLAG__INSENSITIVE;
	case S7: return TOK_FLAG__UNKNOWN;
	case S8: return TOK_NEGATE;
	case S9: return lx->z = z7, TOK_CLOSE;
	case S10: return lx->z = z7, TOK_SUB;
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		if (lx->push != NULL) {
			if (-1 == lx->push(lx->buf_opaque, (char)c)) {
				return TOK_ERROR;
			}
		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z5(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '(': state = S2; break;
			case ')': state = S3; break;
			default: state = S1; break;
			}
			break;

		case S1: /* e.g. "\\x00" */
			switch ((unsigned char) c) {
			case '(':
			case ')': lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z(lx);
			default: break;
			}
			break;

		case S2: /* e.g. "(" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_INVALID__COMMENT;

		case S3: /* e.g. ")" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, lx->z(lx);

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_UNKNOWN;
	case S2: return TOK_INVALID__COMMENT;
	case S3: return lx->z = z7, lx->z(lx);
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S1:
		case S3:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z6(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3, S4, S5, S6, S7, S8, S9, 
		S10, S11, S12, S13, S14, S15, S16, S17, S18, S19, 
		S20, S21, S22, S23, S24, S25, S26, S27, S28, S29, 
		S30, S31, S32, S33, S34, S35, S36, S37, S38, S39, 
		S40, S41, S42, S43, S44, S45, S46, S47, S48, S49, 
		S50, S51, S52, S53, S54, S55, S56, S57, S58, S59, 
		S60, S61, S62, S63, S64, S65, S66, S67, S68, S69, 
		S70, S71, S72, S73, S74, S75, S76, S77, S78
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case ':': state = S1; break;
			case 'L': state = S2; break;
			case 'p': state = S3; break;
			case 'M': state = S4; break;
			case 'T': state = S5; break;
			case 'S': state = S6; break;
			case 'P': state = S7; break;
			case 'A': state = S8; break;
			case 'a': state = S9; break;
			case 'N': state = S10; break;
			case 'C': state = S11; break;
			case 'n': state = S12; break;
			case 'F': state = S13; break;
			case ')': state = S14; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S1: /* e.g. ":" */
			switch ((unsigned char) c) {
			case ')': 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			default: state = S78; break;
			}
			break;

		case S2: /* e.g. "L" */
			switch ((unsigned char) c) {
			case 'F': state = S37; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S3: /* e.g. "p" */
			switch ((unsigned char) c) {
			case 'l': state = S18; break;
			case 'o': state = S76; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S4: /* e.g. "M" */
			switch ((unsigned char) c) {
			case 'A': state = S74; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S5: /* e.g. "T" */
			switch ((unsigned char) c) {
			case 'H': state = S72; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S6: /* e.g. "S" */
			switch ((unsigned char) c) {
			case 'K': state = S70; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S7: /* e.g. "P" */
			switch ((unsigned char) c) {
			case 'R': state = S67; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S8: /* e.g. "A" */
			switch ((unsigned char) c) {
			case 'C': state = S60; break;
			case 'N': state = S61; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S9: /* e.g. "a" */
			switch ((unsigned char) c) {
			case 't': state = S56; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S10: /* e.g. "N" */
			switch ((unsigned char) c) {
			case 'O': state = S46; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S11: /* e.g. "C" */
			switch ((unsigned char) c) {
			case 'R': state = S41; break;
			case 'O': state = S42; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S12: /* e.g. "n" */
			switch ((unsigned char) c) {
			case 'l': state = S18; break;
			case 'e': state = S19; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S13: /* e.g. "F" */
			switch ((unsigned char) c) {
			case ':': state = S1; break;
			case 'A': state = S15; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S14: /* e.g. ")" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z7, lx->z(lx);

		case S15: /* e.g. "FA" */
			switch ((unsigned char) c) {
			case 'I': state = S16; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S16: /* e.g. "FAI" */
			switch ((unsigned char) c) {
			case 'L': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S17: /* e.g. "FAIL" */
			switch ((unsigned char) c) {
			case ':': state = S1; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S18: /* e.g. "nl" */
			switch ((unsigned char) c) {
			case 'a':
			case 'b': state = S36; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S19: /* e.g. "ne" */
			switch ((unsigned char) c) {
			case 'g': state = S20; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S20: /* e.g. "neg" */
			switch ((unsigned char) c) {
			case 'a': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S21: /* e.g. "nega" */
			switch ((unsigned char) c) {
			case 't': state = S22; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S22: /* e.g. "negat" */
			switch ((unsigned char) c) {
			case 'i': state = S23; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S23: /* e.g. "negati" */
			switch ((unsigned char) c) {
			case 'v': state = S24; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S24: /* e.g. "negativ" */
			switch ((unsigned char) c) {
			case 'e': state = S25; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S25: /* e.g. "negative" */
			switch ((unsigned char) c) {
			case '_': state = S26; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S26: /* e.g. "negative_" */
			switch ((unsigned char) c) {
			case 'l': state = S27; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S27: /* e.g. "negative_l" */
			switch ((unsigned char) c) {
			case 'o': state = S28; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S28: /* e.g. "negative_lo" */
			switch ((unsigned char) c) {
			case 'o': state = S29; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S29: /* e.g. "negative_loo" */
			switch ((unsigned char) c) {
			case 'k': state = S30; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S30: /* e.g. "negative_look" */
			switch ((unsigned char) c) {
			case 'b': state = S31; break;
			case 'a': state = S32; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S31: /* e.g. "negative_lookb" */
			switch ((unsigned char) c) {
			case 'e': state = S38; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S32: /* e.g. "negative_looka" */
			switch ((unsigned char) c) {
			case 'h': state = S33; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S33: /* e.g. "negative_lookah" */
			switch ((unsigned char) c) {
			case 'e': state = S34; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S34: /* e.g. "negative_lookahe" */
			switch ((unsigned char) c) {
			case 'a': state = S35; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S35: /* e.g. "negative_lookahea" */
			switch ((unsigned char) c) {
			case 'd': state = S36; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S36: /* e.g. "nla" */
			switch ((unsigned char) c) {
			case ':': state = S37; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S37: /* e.g. "LF" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;

		case S38: /* e.g. "negative_lookbe" */
			switch ((unsigned char) c) {
			case 'h': state = S39; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S39: /* e.g. "negative_lookbeh" */
			switch ((unsigned char) c) {
			case 'i': state = S40; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S40: /* e.g. "negative_lookbehi" */
			switch ((unsigned char) c) {
			case 'n': state = S35; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S41: /* e.g. "CR" */
			switch ((unsigned char) c) {
			case 'L': state = S2; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S42: /* e.g. "CO" */
			switch ((unsigned char) c) {
			case 'M': state = S43; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S43: /* e.g. "COM" */
			switch ((unsigned char) c) {
			case 'M': state = S44; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S44: /* e.g. "COMM" */
			switch ((unsigned char) c) {
			case 'I': state = S45; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S45: /* e.g. "ACCEP" */
			switch ((unsigned char) c) {
			case 'T': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S46: /* e.g. "NO" */
			switch ((unsigned char) c) {
			case '_': state = S47; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S47: /* e.g. "NO_" */
			switch ((unsigned char) c) {
			case 'S': state = S48; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S48: /* e.g. "NO_S" */
			switch ((unsigned char) c) {
			case 'T': state = S49; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S49: /* e.g. "NO_ST" */
			switch ((unsigned char) c) {
			case 'A': state = S50; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S50: /* e.g. "NO_STA" */
			switch ((unsigned char) c) {
			case 'R': state = S51; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S51: /* e.g. "NO_STAR" */
			switch ((unsigned char) c) {
			case 'T': state = S52; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S52: /* e.g. "NO_START" */
			switch ((unsigned char) c) {
			case '_': state = S53; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S53: /* e.g. "NO_START_" */
			switch ((unsigned char) c) {
			case 'O': state = S54; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S54: /* e.g. "NO_START_O" */
			switch ((unsigned char) c) {
			case 'P': state = S55; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S55: /* e.g. "NO_START_OP" */
			switch ((unsigned char) c) {
			case 'T': state = S37; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S56: /* e.g. "at" */
			switch ((unsigned char) c) {
			case 'o': state = S57; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S57: /* e.g. "ato" */
			switch ((unsigned char) c) {
			case 'm': state = S58; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S58: /* e.g. "atom" */
			switch ((unsigned char) c) {
			case 'i': state = S59; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S59: /* e.g. "atomi" */
			switch ((unsigned char) c) {
			case 'c': state = S36; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S60: /* e.g. "AC" */
			switch ((unsigned char) c) {
			case 'C': state = S65; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S61: /* e.g. "AN" */
			switch ((unsigned char) c) {
			case 'Y': state = S62; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S62: /* e.g. "ANY" */
			switch ((unsigned char) c) {
			case 'C': state = S63; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S63: /* e.g. "ANYC" */
			switch ((unsigned char) c) {
			case 'R': state = S64; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S64: /* e.g. "ANYCR" */
			switch ((unsigned char) c) {
			case 'L': state = S2; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S65: /* e.g. "ACC" */
			switch ((unsigned char) c) {
			case 'E': state = S66; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S66: /* e.g. "ACCE" */
			switch ((unsigned char) c) {
			case 'P': state = S45; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S67: /* e.g. "PR" */
			switch ((unsigned char) c) {
			case 'U': state = S68; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S68: /* e.g. "PRU" */
			switch ((unsigned char) c) {
			case 'N': state = S69; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S69: /* e.g. "PRUN" */
			switch ((unsigned char) c) {
			case 'E': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S70: /* e.g. "SK" */
			switch ((unsigned char) c) {
			case 'I': state = S71; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S71: /* e.g. "SKI" */
			switch ((unsigned char) c) {
			case 'P': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S72: /* e.g. "TH" */
			switch ((unsigned char) c) {
			case 'E': state = S73; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S73: /* e.g. "THE" */
			switch ((unsigned char) c) {
			case 'N': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S74: /* e.g. "MA" */
			switch ((unsigned char) c) {
			case 'R': state = S75; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S75: /* e.g. "MAR" */
			switch ((unsigned char) c) {
			case 'K': state = S17; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S76: /* e.g. "po" */
			switch ((unsigned char) c) {
			case 's': state = S77; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S77: /* e.g. "pos" */
			switch ((unsigned char) c) {
			case 'i': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S78: /* e.g. ":\\x00" */
			switch ((unsigned char) c) {
			case ')': lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			default: break;
			}
			break;

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S13: return TOK_UNSUPPORTED;
	case S14: return lx->z = z7, lx->z(lx);
	case S17: return TOK_UNSUPPORTED;
	case S37: return TOK_UNSUPPORTED;
	case S41: return TOK_UNSUPPORTED;
	case S62: return TOK_UNSUPPORTED;
	case S78: return TOK_UNSUPPORTED;
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S14:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

static enum lx_pcre_token
z7(struct lx_pcre_lx *lx)
{
	int has_consumed_input = 0;
	int c;

	assert(lx != NULL);

	if (lx->clear != NULL) {
		lx->clear(lx->buf_opaque);
	}

	lx->start = lx->end;

	void *getc_opaque = (void *)lx;
	enum {
		S0, S1, S2, S3, S4, S5, S6, S7, S8, S9, 
		S10, S11, S12, S13, S14, S15, S16, S17, S18, S19, 
		S20, S21, S22, S23, S24, S25, S26, S27, S28, S29, 
		S30, S31, S32, S33, S34, S35, S36, S37, S38, S39, 
		S40, S41, S42, S43, S44, S45, S46, S47, S48
	} state;

	state = S0;

	while (c = fsm_getc(getc_opaque), c != EOF) {
		has_consumed_input = 1;
		switch (state) {
		case S0: /* e.g. "" */
			switch ((unsigned char) c) {
			case '\\': state = S2; break;
			case '\n':
			case '\r': state = S3; break;
			case '\t':
			case '\v':
			case '\f':
			case ' ': state = S4; break;
			case '#': state = S5; break;
			case '{': state = S6; break;
			case '[': state = S7; break;
			case '(': state = S8; break;
			case '|': state = S9; break;
			case '.': state = S10; break;
			case '+': state = S11; break;
			case '*': state = S12; break;
			case '?': state = S13; break;
			case '$': state = S14; break;
			case '^': state = S15; break;
			case ')': state = S16; break;
			case '\x00': 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			default: state = S1; break;
			}
			break;

		case S1: /* e.g. "\\x01" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CHAR;

		case S2: /* e.g. "\\" */
			switch ((unsigned char) c) {
			case 'Z': state = S14; break;
			case 'A': state = S15; break;
			case 'B':
			case 'C':
			case 'G':
			case 'K':
			case 'X':
			case 'b':
			case 'g':
			case 'k': state = S21; break;
			case '$':
			case '(':
			case ')':
			case '*':
			case '+':
			case '.':
			case '?':
			case '[':
			case '\\':
			case '^':
			case 'a':
			case 'e':
			case 'f':
			case 'n':
			case 'r':
			case 't':
			case '{':
			case '|': state = S28; break;
			case 'E': state = S29; break;
			case 'z': state = S30; break;
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': state = S31; break;
			case 'Q': state = S32; break;
			case 'c': state = S33; break;
			case 'o': state = S34; break;
			case 'x': state = S36; break;
			case 'D':
			case 'H':
			case 'N':
			case 'S':
			case 'V':
			case 'W':
			case 'd':
			case 'h':
			case 's':
			case 'v':
			case 'w': state = S37; break;
			case 'R': state = S38; break;
			case '0': state = S39; break;
			default: state = S35; break;
			}
			break;

		case S3: /* e.g. "\\x0a" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NEWLINE;

		case S4: /* e.g. "\\x09" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_WHITESPACE;

		case S5: /* e.g. "#" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_MAYBE_COMMENT;

		case S6: /* e.g. "{" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z1, TOK_OPENCOUNT;

		case S7: /* e.g. "[" */
			switch ((unsigned char) c) {
			case '^': state = S25; break;
			case ']': state = S26; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z3, TOK_OPENGROUP;
			}
			break;

		case S8: /* e.g. "(" */
			switch ((unsigned char) c) {
			case '?': state = S17; break;
			case '*': state = S18; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OPEN;
			}
			break;

		case S9: /* e.g. "|" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_ALT;

		case S10: /* e.g. "." */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_ANY;

		case S11: /* e.g. "+" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_PLUS;

		case S12: /* e.g. "*" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_STAR;

		case S13: /* e.g. "\077" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OPT;

		case S14: /* e.g. "$" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_END__NL;

		case S15: /* e.g. "^" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_START;

		case S16: /* e.g. ")" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CLOSE;

		case S17: /* e.g. "(\077" */
			switch ((unsigned char) c) {
			case '#': state = S19; break;
			case '<': state = S20; break;
			case '!':
			case '&':
			case '=': state = S21; break;
			case 'P': state = S22; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z4, TOK_FLAGS;
			}
			break;

		case S18: /* e.g. "(*" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z6, lx->z(lx);

		case S19: /* e.g. "(\077#" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z5, lx->z(lx);

		case S20: /* e.g. "(\077<" */
			switch ((unsigned char) c) {
			case '!':
			case '=': state = S21; break;
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'G':
			case 'H':
			case 'I':
			case 'J':
			case 'K':
			case 'L':
			case 'M':
			case 'N':
			case 'O':
			case 'P':
			case 'Q':
			case 'R':
			case 'S':
			case 'T':
			case 'U':
			case 'V':
			case 'W':
			case 'X':
			case 'Y':
			case 'Z':
			case '_':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f':
			case 'g':
			case 'h':
			case 'i':
			case 'j':
			case 'k':
			case 'l':
			case 'm':
			case 'n':
			case 'o':
			case 'p':
			case 'q':
			case 'r':
			case 's':
			case 't':
			case 'u':
			case 'v':
			case 'w':
			case 'x':
			case 'y':
			case 'z': state = S23; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S21: /* e.g. "\\B" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;

		case S22: /* e.g. "(\077P" */
			switch ((unsigned char) c) {
			case '>': state = S21; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S23: /* e.g. "(\077<A" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'G':
			case 'H':
			case 'I':
			case 'J':
			case 'K':
			case 'L':
			case 'M':
			case 'N':
			case 'O':
			case 'P':
			case 'Q':
			case 'R':
			case 'S':
			case 'T':
			case 'U':
			case 'V':
			case 'W':
			case 'X':
			case 'Y':
			case 'Z':
			case '_':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f':
			case 'g':
			case 'h':
			case 'i':
			case 'j':
			case 'k':
			case 'l':
			case 'm':
			case 'n':
			case 'o':
			case 'p':
			case 'q':
			case 'r':
			case 's':
			case 't':
			case 'u':
			case 'v':
			case 'w':
			case 'x':
			case 'y':
			case 'z': break;
			case '>': state = S24; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S24: /* e.g. "(\077<A>" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OPENCAPTURE;

		case S25: /* e.g. "[^" */
			switch ((unsigned char) c) {
			case ']': state = S27; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z3, TOK_OPENGROUPINV;
			}
			break;

		case S26: /* e.g. "[]" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z3, TOK_OPENGROUPCB;

		case S27: /* e.g. "[^]" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z3, TOK_OPENGROUPINVCB;

		case S28: /* e.g. "\\$" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_ESC;

		case S29: /* e.g. "\\E" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z(lx);

		case S30: /* e.g. "\\z" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_END;

		case S31: /* e.g. "\\1" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9': break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_UNSUPPORTED;
			}
			break;

		case S32: /* e.g. "\\Q" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return lx->z = z0, lx->z(lx);

		case S33: /* e.g. "\\c" */
			state = S48; break;

		case S34: /* e.g. "\\o" */
			switch ((unsigned char) c) {
			case '{': state = S46; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NOESC;
			}
			break;

		case S35: /* e.g. "\\\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NOESC;

		case S36: /* e.g. "\\x" */
			switch ((unsigned char) c) {
			case '{': state = S42; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S43; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;
			}
			break;

		case S37: /* e.g. "\\D" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_NAMED__CLASS;

		case S38: /* e.g. "\\R" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_EOL;

		case S39: /* e.g. "\\0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S40; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;
			}
			break;

		case S40: /* e.g. "\\00" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S41; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;
			}
			break;

		case S41: /* e.g. "\\000" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_OCT;

		case S42: /* e.g. "\\x{" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S45; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S43: /* e.g. "\\x0" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': state = S44; break;
			default:  lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;
			}
			break;

		case S44: /* e.g. "\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_HEX;

		case S45: /* e.g. "\\x{0" */
			switch ((unsigned char) c) {
			case '}': state = S44; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7':
			case '8':
			case '9':
			case 'A':
			case 'B':
			case 'C':
			case 'D':
			case 'E':
			case 'F':
			case 'a':
			case 'b':
			case 'c':
			case 'd':
			case 'e':
			case 'f': break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S46: /* e.g. "\\o{" */
			switch ((unsigned char) c) {
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': state = S47; break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S47: /* e.g. "\\o{0" */
			switch ((unsigned char) c) {
			case '}': state = S41; break;
			case '0':
			case '1':
			case '2':
			case '3':
			case '4':
			case '5':
			case '6':
			case '7': break;
			default:  
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
			}
			break;

		case S48: /* e.g. "\\c\\x00" */
			lx_pcre_ungetc(lx, c); lx_pcre_dynpop(lx->buf_opaque); return TOK_CONTROL;

		default:
			; /* unreached */
		}
	}

	/* end states */
	switch (state) {
	case S1: return TOK_CHAR;
	case S2: return TOK_INVALID;
	case S3: return TOK_NEWLINE;
	case S4: return TOK_WHITESPACE;
	case S5: return TOK_MAYBE_COMMENT;
	case S6: return lx->z = z1, TOK_OPENCOUNT;
	case S7: return lx->z = z3, TOK_OPENGROUP;
	case S8: return TOK_OPEN;
	case S9: return TOK_ALT;
	case S10: return TOK_ANY;
	case S11: return TOK_PLUS;
	case S12: return TOK_STAR;
	case S13: return TOK_OPT;
	case S14: return TOK_END__NL;
	case S15: return TOK_START;
	case S16: return TOK_CLOSE;
	case S17: return lx->z = z4, TOK_FLAGS;
	case S18: return lx->z = z6, lx->z(lx);
	case S19: return lx->z = z5, lx->z(lx);
	case S21: return TOK_UNSUPPORTED;
	case S24: return TOK_OPENCAPTURE;
	case S25: return lx->z = z3, TOK_OPENGROUPINV;
	case S26: return lx->z = z3, TOK_OPENGROUPCB;
	case S27: return lx->z = z3, TOK_OPENGROUPINVCB;
	case S28: return TOK_ESC;
	case S29: return TOK_EOF;
	case S30: return TOK_END;
	case S31: return TOK_UNSUPPORTED;
	case S32: return lx->z = z0, lx->z(lx);
	case S33: return TOK_NOESC;
	case S34: return TOK_NOESC;
	case S35: return TOK_NOESC;
	case S36: return TOK_HEX;
	case S37: return TOK_NAMED__CLASS;
	case S38: return TOK_EOL;
	case S39: return TOK_OCT;
	case S40: return TOK_OCT;
	case S41: return TOK_OCT;
	case S43: return TOK_HEX;
	case S44: return TOK_HEX;
	case S48: return TOK_CONTROL;
	default: 
				if (!has_consumed_input) { return TOK_EOF; }
				lx_pcre_ungetc(lx, c); lx->lgetc = NULL; return TOK_UNKNOWN;
	}

		switch (state) {
		case S18:
		case S19:
		case S29:
		case S32:
			break;

		default:
			if (lx->push != NULL) {
				if (-1 == lx->push(lx->buf_opaque, (char)c)) {
					return TOK_ERROR;
				}
			}
			break;

		}

	lx->lgetc = NULL;

	if (!has_consumed_input) {
		return TOK_EOF;
	} 
	return TOK_ERROR;
}

const char *
lx_pcre_name(enum lx_pcre_token t)
{
	switch (t) {
	case TOK_NEWLINE: return "NEWLINE";
	case TOK_WHITESPACE: return "WHITESPACE";
	case TOK_MAYBE_COMMENT: return "MAYBE_COMMENT";
	case TOK_COUNT: return "COUNT";
	case TOK_SEP: return "SEP";
	case TOK_CLOSECOUNT: return "CLOSECOUNT";
	case TOK_OPENCOUNT: return "OPENCOUNT";
	case TOK_CHAR: return "CHAR";
	case TOK_RANGE: return "RANGE";
	case TOK_CLOSEGROUPRANGE: return "CLOSEGROUPRANGE";
	case TOK_CLOSEGROUP: return "CLOSEGROUP";
	case TOK_OPENGROUPINVCB: return "OPENGROUPINVCB";
	case TOK_OPENGROUPCB: return "OPENGROUPCB";
	case TOK_OPENGROUPINV: return "OPENGROUPINV";
	case TOK_OPENGROUP: return "OPENGROUP";
	case TOK_FLAG__IGNORE: return "FLAG__IGNORE";
	case TOK_FLAG__EXTENDED: return "FLAG__EXTENDED";
	case TOK_FLAG__SINGLE: return "FLAG__SINGLE";
	case TOK_FLAG__INSENSITIVE: return "FLAG__INSENSITIVE";
	case TOK_FLAG__UNKNOWN: return "FLAG__UNKNOWN";
	case TOK_NEGATE: return "NEGATE";
	case TOK_SUB: return "SUB";
	case TOK_FLAGS: return "FLAGS";
	case TOK_INVALID__COMMENT: return "INVALID__COMMENT";
	case TOK_INVALID: return "INVALID";
	case TOK_NOESC: return "NOESC";
	case TOK_HEX: return "HEX";
	case TOK_OCT: return "OCT";
	case TOK_EOL: return "EOL";
	case TOK_NAMED__CLASS: return "NAMED__CLASS";
	case TOK_CONTROL: return "CONTROL";
	case TOK_ESC: return "ESC";
	case TOK_ALT: return "ALT";
	case TOK_ANY: return "ANY";
	case TOK_PLUS: return "PLUS";
	case TOK_STAR: return "STAR";
	case TOK_OPT: return "OPT";
	case TOK_UNSUPPORTED: return "UNSUPPORTED";
	case TOK_END: return "END";
	case TOK_END__NL: return "END__NL";
	case TOK_START: return "START";
	case TOK_OPENCAPTURE: return "OPENCAPTURE";
	case TOK_CLOSE: return "CLOSE";
	case TOK_OPEN: return "OPEN";
	case TOK_EOF:     return "EOF";
	case TOK_ERROR:   return "ERROR";
	case TOK_UNKNOWN: return "UNKNOWN";
	default: return "?";
	}
}

const char *
lx_pcre_example(enum lx_pcre_token (*z)(struct lx_pcre_lx *), enum lx_pcre_token t)
{
	assert(z != NULL);

	if (z == z0) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z1) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z2) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z3) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z4) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z5) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z6) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	} else
	if (z == z7) {
		switch (t) {
		case TOK_NEWLINE: return "";
		case TOK_WHITESPACE: return "";
		case TOK_MAYBE_COMMENT: return "";
		case TOK_COUNT: return "";
		case TOK_SEP: return "";
		case TOK_CLOSECOUNT: return "";
		case TOK_OPENCOUNT: return "";
		case TOK_CHAR: return "";
		case TOK_RANGE: return "";
		case TOK_CLOSEGROUPRANGE: return "";
		case TOK_CLOSEGROUP: return "";
		case TOK_OPENGROUPINVCB: return "";
		case TOK_OPENGROUPCB: return "";
		case TOK_OPENGROUPINV: return "";
		case TOK_OPENGROUP: return "";
		case TOK_FLAG__IGNORE: return "";
		case TOK_FLAG__EXTENDED: return "";
		case TOK_FLAG__SINGLE: return "";
		case TOK_FLAG__INSENSITIVE: return "";
		case TOK_FLAG__UNKNOWN: return "";
		case TOK_NEGATE: return "";
		case TOK_SUB: return "";
		case TOK_FLAGS: return "";
		case TOK_INVALID__COMMENT: return "";
		case TOK_INVALID: return "";
		case TOK_NOESC: return "";
		case TOK_HEX: return "";
		case TOK_OCT: return "";
		case TOK_EOL: return "";
		case TOK_NAMED__CLASS: return "";
		case TOK_CONTROL: return "";
		case TOK_ESC: return "";
		case TOK_ALT: return "";
		case TOK_ANY: return "";
		case TOK_PLUS: return "";
		case TOK_STAR: return "";
		case TOK_OPT: return "";
		case TOK_UNSUPPORTED: return "";
		case TOK_END: return "";
		case TOK_END__NL: return "";
		case TOK_START: return "";
		case TOK_OPENCAPTURE: return "";
		case TOK_CLOSE: return "";
		case TOK_OPEN: return "";
		default: goto error;
		}
	}

error:

	errno = EINVAL;
	return NULL;
}

void
lx_pcre_init(struct lx_pcre_lx *lx)
{
	static const struct lx_pcre_lx lx_default;

	assert(lx != NULL);

	*lx = lx_default;

	lx->c = EOF;
	lx->z = z7;

	lx->end.byte = 0;
	lx->end.line = 1;
	lx->end.col  = 1;
	(void)lx_pcre_dynpop;
}

enum lx_pcre_token
lx_pcre_next(struct lx_pcre_lx *lx)
{
	enum lx_pcre_token t;

	assert(lx != NULL);
	assert(lx->z != NULL);

	if (lx->lgetc == NULL) {
		return TOK_EOF;
	}

	t = lx->z(lx);

	if (lx->push != NULL) {
		if (-1 == lx->push(lx->buf_opaque, '\0')) {
			return TOK_ERROR;
		}
	}

	return t;
}

