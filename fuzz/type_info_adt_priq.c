...
alloc(struct theft *t, void *type_env, void **output)
{
	...
	if (env->id_bits == 0) { env->id_bits = env->count_bits - 2; }
	if (env->cost_bits == 0) { env->cost_bits = 10; }

	size_t count = theft_random_bits(t, env->count_bits) + 1;

	struct priq_scenario *res = NULL;
...
{
	(void)type_env;
	struct priq_scenario *scen = (struct priq_scenario *)instance;
	for (size_t i = 0; i < scen->count; i++) {
		const struct priq_op *op = &scen->ops[i];
		switch (op->t) {
		case PRIQ_OP_POP:
			fprintf(f, "%zd: POP\n", i);
			break;
